    ...
    if (conflictingOptions.length > 0) {
      throw new Error(
        `The following options from ${DialectClass.name} conflict with built-in Sequelize options: ${(0, import_utils.join)(
          (0, import_utils.map)(conflictingOptions, (option) => (0, import_ansis.red)(option)),
          ", "
        )}.
    ...
    if (unseenKeys.size > 0) {
      ...
      throw new Error(
        `The following options are not recognized by Sequelize nor ${DialectClass.name}: ${(0, import_utils.join)(
          (0, import_utils.map)(unseenKeys, (option) => (0, import_ansis.red)(option)),
          ", "
        )}.

Sequelize accepts the following options: ${allSequelizeOptionNames.sort(caseInsensitiveEnComparator).map((option) => (0, import_ansis.cyan)(option)).join(", ")}.

${DialectClass.name} accepts the following options (in addition to the Sequelize options): ${[
          ...dialectOptionNames
        ].sort(caseInsensitiveEnComparator).map((option) => (0, import_ansis.cyan)(option)).join(", ")}.
${DialectClass.name} options can be set at the root of the option bag, like Sequelize options.

The following options can be used to configure the connection to the database: ${connectionOptionNames.sort(caseInsensitiveEnComparator).map((option) => (0, import_ansis.cyan)(option)).join(", ")}.
